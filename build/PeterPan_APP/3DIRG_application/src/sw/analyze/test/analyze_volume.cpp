...
ImageType::Pointer loadPNGStack(const std::string& folderPath) {
    ...
    sort(files.begin(), files.end());  // ensure slice order
...
int main(int argc, char** argv) {
    ...
    if (maskMode == "zero") {
        maskF = makeMaskZeroBased(fixed);
        maskM = makeMaskZeroBased(moving);
    } else if (maskMode == "otsu") {
        maskF = makeMaskOtsu(fixed);
        maskM = makeMaskOtsu(moving);
    } else if (maskMode == "pct") {
        maskF = makeMaskPercentile(fixed, pctValue);
        maskM = makeMaskPercentile(moving, pctValue);
    } else {
    ...
    // here I am rewriting the code to use openMP for parallelization
    int maxZ = (int)size[2] - Dz;

#pragma omp parallel
    {
        // thread-local copies (no shared writes inside the loop)
...
// merge per-thread result into global best
#pragma omp critical
        {
